...
class Mocker {
  ...
  async simulateMarketBehavior(
    ...
    while (this.isSimulationRunning) {
      ...
      // 添加自动平衡机制 - 在simulateMarketBehavior中添加
      // 当市场严重不平衡时调整行为
      const totalOrders = this.pendingOrders.length;
      const sellOrders = this.pendingOrders.filter(
        (order) => !order.isBuyOrder,
      ).length;
      const buyOrders = totalOrders - sellOrders;

      // 如果卖单过多，增加买单概率
      let adjustedBuyProbability = BUY_PROBABILITY;
      if (sellOrders > buyOrders * 3) {
        // 卖单是买单的3倍以上
        adjustedBuyProbability = 0.8; // 80%概率创建买单
      }
      // 如果买单过多，增加卖单概率
      else if (buyOrders > sellOrders * 3) {
        adjustedBuyProbability = 0.2; // 20%概率创建买单(80%卖单)
      }
    }
  }
